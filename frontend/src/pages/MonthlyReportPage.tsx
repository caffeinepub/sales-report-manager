...
export default function MonthlyReportPage() {
  ...
  const { data: entries = [], isLoading } = useGetMonthlyEntries(selectedMonth, selectedYear);
    ...
    <div>
      ...
      {isLoading && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
          <RefreshCw size={14} className="animate-spin" /> Loading monthly data...
        </div>
      )}
            ...
            <tbody>
              {aggregated.map((row) => (
                <tr key={row.salesman}>
                  ...
                  <td className="mono text-right">{formatIndianCurrency(row.totalSalesValue)}</td>
                  <td className="mono text-right">{formatIndianCurrency(row.totalDaySales)}</td>
                  <td className="mono text-right">{row.totalDaysWorked}</td>
                  <td className="mono text-right">{formatIndianCurrency(row.avgDailySales)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="total-row">
                ...
                <td className="mono text-right font-bold">{formatIndianCurrency(grandTotals.totalSalesValue)}</td>
                <td className="mono text-right font-bold">{formatIndianCurrency(grandTotals.totalDaySales)}</td>
                <td className="mono text-right font-bold">{grandTotals.totalDaysWorked}</td>
                <td className="mono text-right font-bold">
                  {formatIndianCurrency(
                    grandTotals.totalDaysWorked > 0
                      ? grandTotals.totalSalesValue / grandTotals.totalDaysWorked
                      : 0
                  )}
                </td>
              </tr>
          ...
          <div className="card-erp overflow-hidden">
            <div className="overflow-x-auto">
              <table className="erp-table sticky-header">
                ...
                <tbody>
                  {[...entries]
                    .sort((a, b) => a.date.localeCompare(b.date) || a.salesman.localeCompare(b.salesman))
                    .map((entry, idx) => (
                      <tr key={idx}>
                        <td className="text-xs">{format(new Date(entry.date), 'dd/MM/yyyy (EEE)')}</td>
                        <td className="font-semibold text-xs">{entry.salesman}</td>
                        <td className="text-xs">{entry.loadNumber || '-'}</td>
                        <td className="mono text-right">{Number(entry.totalBills)}</td>
                        <td className="mono text-right">{formatIndianCurrency(entry.salesValue)}</td>
                        <td className="mono text-right">{formatIndianCurrency(entry.daySales)}</td>
                        <td className="mono text-right">{formatIndianCurrency(entry.stockValue)}</td>
                        <td className="mono text-right">{Number(entry.stockQty)}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
