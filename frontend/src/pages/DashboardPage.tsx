...
import { SALESMEN, getBeatForDay, isSunday, SalesmanName } from '../utils/beatSchedule';
...
import { useGetDailyEntries, useSaveDailyEntries, getLocalDailyEntry } from '../hooks/useQueries';
...
export default function DashboardPage() {
  ...
  const [rows, setRows] = useState<RowData[]>(() => buildDefaultRows(new Date()));
  ...
  useEffect(() => {
    ...
    const mergedRows = defaultRows.map((defaultRow) => {
      // Try backend first
      const backendEntry = backendEntries?.find((e) => e.salesman === defaultRow.salesman);
  ...
  const updateRow = useCallback((salesman: string, field: keyof RowData, value: string) => {
    setRows((prev) =>
      prev.map((row) => (row.salesman === salesman ? { ...row, [field]: value } : row))
    );
    setSaveSuccess(false);
  ...
  const validateAll = (): boolean => {
    ...
    rows.forEach((row) => {
      ...
      if (!ln.valid) { rowErrors.loadNumber = ln.error; valid = false; }
      const tb = validateTotalBills(row.totalBills);
      if (!tb.valid) { rowErrors.totalBills = tb.error; valid = false; }
      const sv = validateSalesValue(row.salesValue);
      if (!sv.valid) { rowErrors.salesValue = sv.error; valid = false; }
      const sq = validateStockQty(row.stockQty);
      if (!sq.valid) { rowErrors.stockQty = sq.error; valid = false; }
      if (Object.keys(rowErrors).length > 0) newErrors[row.salesman] = rowErrors;
    });
    setErrors(newErrors);
    return valid;
  };
  ...
  const handleSave = async () => {
    ...
    const entries: DailySalesEntry[] = rows.map((row) => ({
      ...
      totalBills: BigInt(parseInt(row.totalBills) || 0),
      salesValue: parseFloat(row.salesValue) || 0,
      daySales: parseFloat(row.daySales) || 0,
      stockValue: parseFloat(row.stockValue) || 0,
      stockQty: BigInt(parseInt(row.stockQty) || 0),
    }));
  ...
  const totals = rows.reduce(
    (acc, row) => ({
      totalBills: acc.totalBills + (parseInt(row.totalBills) || 0),
      salesValue: acc.salesValue + (parseFloat(row.salesValue) || 0),
      daySales: acc.daySales + (parseFloat(row.daySales) || 0),
      stockValue: acc.stockValue + (parseFloat(row.stockValue) || 0),
      stockQty: acc.stockQty + (parseInt(row.stockQty) || 0),
    }),
  ...
  const rowsWithCumulative = rows.map((row) => {
    cumulativeSales += parseFloat(row.salesValue) || 0;
    ...
    <div>
        ...
        <div className="flex items-center gap-3">
          ...
          {!isWeeklyOff && (
            ...
            >
              ...
              {saveMutation.isPending ? 'Saving...' : saveSuccess ? 'Saved!' : 'Save Daily Data'}
            </button>
          )}
        </div>
      </div>
      ...
      {isWeeklyOff ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-xl font-bold mb-2" style={{ color: 'oklch(0.22 0.07 240)' }}>
            Weekly Off
          </h3>
          <p className="text-muted-foreground max-w-sm">
            Sunday is a weekly off day. No sales data entry is required. Please select a weekday (Monday–Saturday) to enter data.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="mb-4 flex items-center gap-2">
            <span
              ...
              style={{ backgroundColor: 'oklch(0.55 0.18 200)', color: 'white' }}
            >
              ...
              <table className="erp-table sticky-header">
                ...
                <tbody>
                  {rowsWithCumulative.map((row) => {
                    ...
                    return (
                      <tr key={row.salesman}>
                        <td className="font-semibold text-xs" style={{ color: 'oklch(0.22 0.07 240)' }}>
                          {row.salesman}
                        </td>
                        <td className="text-xs font-medium" style={{ color: 'oklch(0.40 0.10 200)' }}>
                          {row.beat}
                        </td>
                        <td>
                          <input
                            ...
                            onChange={(e) => updateRow(row.salesman, 'loadNumber', e.target.value)}
                            className={`erp-input ${rowErr.loadNumber ? 'error' : ''}`}
                            placeholder="L001"
                          />
                          {rowErr.loadNumber && (
                            <p className="text-xs text-destructive mt-0.5">{rowErr.loadNumber}</p>
                          )}
                        </td>
                        <td>
                          <input
                            ...
                            onChange={(e) => updateRow(row.salesman, 'totalBills', e.target.value)}
                            className={`erp-input ${rowErr.totalBills ? 'error' : ''}`}
                          />
                          {rowErr.totalBills && (
                            <p className="text-xs text-destructive mt-0.5">{rowErr.totalBills}</p>
                          )}
                        </td>
                        <td>
                          <input
                            ...
                            onChange={(e) => updateRow(row.salesman, 'salesValue', e.target.value)}
                            className={`erp-input ${rowErr.salesValue ? 'error' : ''}`}
                          />
                          {rowErr.salesValue && (
                            <p className="text-xs text-destructive mt-0.5">{rowErr.salesValue}</p>
                          )}
                        </td>
                        <td>
                          <input
                            ...
                            step="0.01"
                            value={row.daySales}
                            onChange={(e) => updateRow(row.salesman, 'daySales', e.target.value)}
                            className="erp-input"
                          />
                        </td>
                        <td>
                          <input
                            ...
                            onChange={(e) => updateRow(row.salesman, 'stockValue', e.target.value)}
                            className="erp-input"
                          />
                        </td>
                        <td>
                          <input
                            ...
                            min="0"
                            value={row.stockQty}
                            onChange={(e) => updateRow(row.salesman, 'stockQty', e.target.value)}
                            className={`erp-input ${rowErr.stockQty ? 'error' : ''}`}
                          />
                          {rowErr.stockQty && (
                            <p className="text-xs text-destructive mt-0.5">{rowErr.stockQty}</p>
                          )}
                        </td>
                        <td className="font-semibold mono text-right" style={{ color: 'oklch(0.35 0.12 145)' }}>
                ...
                <tfoot>
                  <tr className="total-row">
                    ...
                    <td className="font-bold mono">{formatIndianCurrency(totals.salesValue)}</td>
                    <td className="font-bold mono">{formatIndianCurrency(totals.daySales)}</td>
                    <td className="font-bold mono">{formatIndianCurrency(totals.stockValue)}</td>
          ...
          <div className="grid grid-cols-4 gap-4 mt-6">
            {[
              { label: 'Total Bills', value: totals.totalBills.toString(), color: 'oklch(0.22 0.07 240)' },
              { label: 'Total Sales Value', value: formatIndianCurrency(totals.salesValue), color: 'oklch(0.35 0.12 145)' },
              { label: 'Total Day Sales', value: formatIndianCurrency(totals.daySales), color: 'oklch(0.55 0.18 200)' },
              { label: 'Total Stock Value', value: formatIndianCurrency(totals.stockValue), color: 'oklch(0.65 0.18 60)' },
            ].map((card) => (
              <div key={card.label} className="card-erp p-4">
                <div className="text-xs text-muted-foreground mb-1">{card.label}</div>
                <div className="text-lg font-bold mono" style={{ color: card.color }}>
                  {card.value}
                </div>
              </div>
            ))}
