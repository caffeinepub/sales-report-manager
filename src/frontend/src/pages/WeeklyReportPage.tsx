  ...
  return SALESMEN.map((salesman) => {
    const days: (DayEntry | null)[] = weekDays.map((day) => {
      ...
      const entry = entries.find(
        (e) => e.salesman === salesman && e.date === dateStr,
      );
    ...
    <div>
      <div className="page-header">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            onChange={(e) =>
              setWeekDate(new Date(`${e.target.value}T00:00:00`))
            }
            className="erp-input"
            style={{ width: "150px" }}
          />
      ...
      {isLoading && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
          <RefreshCw size={14} className="animate-spin" /> Loading weekly
          data...
        </div>
      )}
              ...
              <table className="erp-table">
                  ...
                  {smData.days.map((day, idx) => (
                    <tr key={day?.date ?? idx}>
                      ...
                      <td className="text-xs">
                        {day ? format(new Date(day.date), "dd/MM/yyyy") : "-"}
                      </td>
                      <td
                        className="text-xs"
                      ...
                      <td className="mono text-right">
                        {formatIndianCurrency(day?.salesValue || 0)}
                      </td>
                      <td className="mono text-right">
                        {formatIndianCurrency(day?.daySales || 0)}
                      </td>
                      <td className="mono text-right">
                        {formatIndianCurrency(day?.stockValue || 0)}
                      </td>
                ...
                <tfoot>
                  <tr className="subtotal-row">
                    <td colSpan={4} className="font-semibold text-xs">
                      Weekly Subtotal
                    </td>
                    <td className="mono text-right font-semibold">
                      {smData.totals.totalBills}
                    </td>
                    <td className="mono text-right font-semibold">
                      {formatIndianCurrency(smData.totals.salesValue)}
                    </td>
                    <td className="mono text-right font-semibold">
                      {formatIndianCurrency(smData.totals.daySales)}
                    </td>
                    <td className="mono text-right font-semibold">
                      {formatIndianCurrency(smData.totals.stockValue)}
                    </td>
                    <td className="mono text-right font-semibold">
                      {smData.totals.stockQty}
                    </td>
                  </tr>
                </tfoot>
              </table>
            ...
            <table className="erp-table">
              ...
              <tbody>
                {weekData.map((sm) => (
                  <tr key={sm.salesman}>
                    ...
                    <td className="mono text-right">
                      {formatIndianCurrency(sm.totals.salesValue)}
                    </td>
                    <td className="mono text-right">
                      {formatIndianCurrency(sm.totals.daySales)}
                    </td>
                    <td className="mono text-right">
                      {formatIndianCurrency(sm.totals.stockValue)}
                    </td>
              ...
              <tfoot>
                <tr className="total-row">
                  ...
                  <td className="mono text-right font-bold">
                    {formatIndianCurrency(grandTotals.salesValue)}
                  </td>
                  <td className="mono text-right font-bold">
                    {formatIndianCurrency(grandTotals.daySales)}
                  </td>
                  <td className="mono text-right font-bold">
                    {formatIndianCurrency(grandTotals.stockValue)}
                  </td>
                  <td className="mono text-right font-bold">
                    {grandTotals.stockQty}
                  </td>
                </tr>
              </tfoot>
            </table>
