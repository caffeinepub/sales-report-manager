...
export default function ExportReportsPage() {
  ...
  const { data: monthlyEntries = [] } = useGetMonthlyEntries(
    selectedMonth,
    selectedYear,
  );
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 p-1 rounded-lg border border-border bg-muted w-fit">
        {(["daily", "weekly", "monthly", "stock"] as ReportType[]).map(
          (type) => (
            <button
              type="button"
              key={type}
      ...
      <div className="card-erp p-4 mb-6">
        ...
        {reportType === "weekly" && (
          <div className="flex items-center gap-3">
            <label htmlFor="export-week-date" className="text-sm font-medium">
              Any date in week:
            </label>
            <input
              id="export-week-date"
        ...
        {reportType === "monthly" && (
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="export-year-select" className="text-sm font-medium">
              Year:
            </label>
            <select
      ...
      <div className="grid grid-cols-2 gap-4">
        {reportType === "daily" && (
          <ExportCard
            ...
            onExportPDF={() =>
              withExport(() => exportDailyPDF(dailyEntries, selectedDate))
            }
            onExportExcel={() =>
              withExport(() => exportDailyExcel(dailyEntries, selectedDate))
            }
          />
        )}
        ...
        {reportType === "weekly" && (
          <ExportCard
            ...
            onExportPDF={() =>
              withExport(() =>
                exportWeeklyPDF(weeklyEntries, weekStart, weekEnd),
              )
            }
            onExportExcel={() =>
              withExport(() =>
                exportWeeklyExcel(weeklyEntries, weekStart, weekEnd),
              )
            }
          />
        )}
        ...
        <div className="card-erp p-5 flex flex-col justify-between">
          <div>
            ...
            <ul className="text-xs text-muted-foreground space-y-1.5">
              <li className="flex items-center gap-2">
                ...
                PDF exports include company header, formatted tables, and page
                numbers
              </li>
              <li className="flex items-center gap-2">
                <FileSpreadsheet
              ...
              <li className="flex items-center gap-2">
                ...
                All monetary values are formatted with ₹ symbol (Indian
                numbering)
              </li>
            </ul>
          </div>
          ...
          >
            <strong>Tip:</strong> Make sure to save your daily data before
            exporting to ensure the latest entries are included in the report.
          </div>
        </div>
      </div>
